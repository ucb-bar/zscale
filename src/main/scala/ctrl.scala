...
class Control extends Module with ZscaleParameters
{
  ...
  val decode_table: Array[(BitPat, List[BitPat])] = Array(
      ...
      SLLI->    List(Y, N, N, Y, N, CSR.N, A1_RS1,  A2_IMM,  IMM_I,  FN_SL,     Y, N, N, X, MT_X,  N),
      SRLI->    List(Y, N, N, Y, N, CSR.N, A1_RS1,  A2_IMM,  IMM_I,  FN_SR,     Y, N, N, X, MT_X,  N),
      SRAI->    List(Y, N, N, Y, N, CSR.N, A1_RS1,  A2_IMM,  IMM_I,  FN_SRA,    Y, N, N, X, MT_X,  N),
      ...
      FENCE_I-> List(Y, N, N, N, Y, CSR.N, A1_X,    A2_X,    IMM_X,  FN_X,      N, N, N, X, MT_X,  N),

      SCALL->   List(Y, N, N, N, N, CSR.I, A1_X,    A2_X,    IMM_X,  FN_X,      N, N, N, X, MT_X,  N),
      SBREAK->  List(Y, N, N, N, N, CSR.I, A1_X,    A2_X,    IMM_X,  FN_X,      N, N, N, X, MT_X,  N),
      SRET->    List(Y, N, N, N, N, CSR.I, A1_X,    A2_X,    IMM_X,  FN_X,      N, N, N, X, MT_X,  N),
